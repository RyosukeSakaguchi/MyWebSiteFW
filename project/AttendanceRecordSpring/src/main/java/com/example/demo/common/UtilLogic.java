...
public class UtilLogic {
	...
	public static int getMonthlySalary(String loginId, String position, int year, int month){
		...
		//それらをint型に直して(ex : 21:34:52 →　213452)給与を計算。給料の計算の際はdouble型で行うが、給料は整数なのでint型にキャスト
	...
	/** 受け取ったユーザーリストで「勤務中のユーザー」を先に並べ、「帰宅しているユーザー」を後に並べかえ、
	 *  さらに管理者がリストに入っている場合は取り除く
	 * @param userList
	 ...
	 * @return int
	 */
	public static int getTotalPageNumber(List<UserBeans> userList, int userNumberPerPage) {
